...
namespace Bolt {
    ...
    // TImageSamplerImplPtr CombinedImageSamplerTextureBuilder::buildCubemapFromPath(char* path)
    // {
        
    // }
    TImageSamplerImplPtr CombinedImageSamplerTextureBuilder::buildCubemapFromPixels(const unsigned char* pixels[6], uint32_t width, uint32_t height)
    {
        VkDevice device = VulkanRenderer::get()->getDevice();
        VkAllocationCallbacks* callbacks = VulkanRenderer::get()->getAllocationCallbacks();
        VkSamplerCreateInfo samplerInfo{};
        ...
        VulkanTexture* texture = QUAINT_NEW(m_context, VulkanTexture, m_context);
        //TODO: Hardcoding flag for now. Create a comming enum for that later
        VulkanRenderer::get()->createCubemapFromPixels(*texture, pixels, width, height, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
        VulkanTextureRef texRef(texture, Quaint::Deleter<VulkanTexture>(m_context));
