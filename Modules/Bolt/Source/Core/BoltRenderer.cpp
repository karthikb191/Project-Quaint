...
private:
    ...
    void createSurface() {
        ...
        assert(window != nullptr && "Invalid window retrieved");
         ...
         VkResult res = vkCreateWin32SurfaceKHR(instance, &winSurfaceInfo, nullptr, &surface);
    ...
    void pickPhysicalDevice() {
        ...
        for (const auto& device : devices) {
            if (isDeviceSuitable(device)) {
                ...
                std::cout << "Selected device: " << prop.deviceName << "\n"; 
    ...
    void createLogicalDevice() {
        ...
        std::set<uint32_t> uniqueQueueFamilies = {indices.graphics.get(), indices.presentation.get()};
        ...
        vkGetDeviceQueue(device, indices.graphics.get(), 0, &graphicsQueue);
        vkGetDeviceQueue(device, indices.presentation.get(), 0, &presentQueue);
    }
    ...
    void createSwapChain() {
        ...
        QueueFamilies indices = findQueueFamilies(physicalDevice);
        uint32_t queueFamilyIndices[] = {indices.graphics.get(), indices.presentation.get()};

        if (indices.graphics.get() != indices.presentation.get()) {
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("D:\\Works\\Project-Quaint\\Data\\Shaders\\TestTriangle\\simpleTri.vert.spv");
        auto fragShaderCode = readFile("D:\\Works\\Project-Quaint\\Data\\Shaders\\TestTriangle\\simpleTri.frag.spv");
    ...
    void createCommandPool() {
        ...
        poolInfo.queueFamilyIndex = queueFamilyIndices.graphics.get();
    ...
    bool isDeviceSuitable(VkPhysicalDevice device) {
        ...
        return indices.allSet() && extensionsSupported && swapChainAdequate;
    }
...
namespace Bolt
{
    ...
    void BoltRenderer::update()
    {
        ...
        while (true)
        {
            ...
            //std::cout << "Bolt Render\n";
            //m_window.getWindowsWindow()->update();
#ifdef RUNAPP
            app.run();
#else
